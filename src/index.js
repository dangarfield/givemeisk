...
const sendMessage = async (isDryRun, logEle, task) => {
  ...
  if (!task.prizeMessage) {
    ...
    const embed = {
      title: task.perPrize > 1 ? `${task.perPrize}x ${task.prizeName}` : task.prizeName,
      description: msg,
      color: 0xFAD02C,
      thumbnail: {
      ...
      // footer: {
      //   text: 'To claim prizes: do something' // TODO - Add this
      // }
    }
  ...
  if (!isDryRun) {
    const messageRes = await executeFetchPost(`${BASE_URL}/channels/${persistedData.channel.id}/messages`, JSON.stringify(paramsBody))// .map(m => { return { id: m.user.id, name: m.user.globalName || m.user.username, blacklisted: false } })
...
const calculateAndPresentResults = async () => {
  ...
  for (const line of prizeList) {
    for (let i = 0; i < line.noPrizes; i += line.perPrize) {
      ...
      giveawayList.push({ order: i + 1, giveawayOrder: Math.random(), prizeName: line.prizeName, perPrize: line.perPrize, prizeMessage, sponsorMessage: line.sponsorMessage, winnerDiscordId: winner.id, winnerDiscordName: winner.name, msgSent: false })
    }
  }
