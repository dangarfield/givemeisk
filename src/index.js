...
const sendMessage = async (isDryRun, logEle, task) => {
  ...
  if (!task.prizeMessage) {
    ...
    const embed = {
      ...
      footer: {
        text: 'To claim prizes: Make sure you link your eve account! Use the slash command `/givemeisk` and the giveaway code `skullyisinsane`!'
      }
    }
  ...
  if (!isDryRun) {
    ...
    // Removing DMs and using slash commands
    // if (pm !== undefined) {
